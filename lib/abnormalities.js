	...
	constructor(mod) {
		...
		let abnormalityUpdate = (type, event) => {
			if(event.target === this.player.gameId) {
				...
				// 30 day duration items will return overflow, set this to permanent until server returns a lower number
				if (event.duration > 2147483647 || event.duration < 0) event.duration = 2147483647;
