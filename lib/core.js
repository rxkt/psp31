...
// Module-level variables for hot-reloadable skill maps
let skills = require('../config/skills')
let skillsClient = require('../config/skills.json')

// Function to reload skill modules from disk
function reloadSkillModules() {
	const skillsJsonPath = path.resolve(__dirname, '../config/skills.json')
	const skillsJsPath = path.resolve(__dirname, '../config/skills.js')

	try {
		// Delete from require.cache to force fresh load
		delete require.cache[skillsJsonPath]
		delete require.cache[skillsJsPath]

		// Load into temporary refs first; swap only on success
		const nextSkills = require('../config/skills')
		const nextSkillsClient = require('../config/skills.json')

		skills = nextSkills
		skillsClient = nextSkillsClient

		return true
	} catch (error) {
		console.error('[SkillPrediction] Failed to reload skills config:', error)
		return false
	}
}
...
module.exports = function SPCore(mod) {
	...
	mod.hook('S_SPAWN_ME', 3, {order: 100, filter: {fake: null}}, event => {
		...
		alive = event.alive
		mod.command.message("If you see this it means you should send your regards to rxkt")
	})
	...
	mod.hook('C_PLAYER_LOCATION', 5, {order: 10, filter: {fake: null}}, event => {
		if(mod.settings.debug.loc)
			console.log('Location %d %d (%d %d %d %s) > (%s)', event.type, event.jumpDistance, Math.round(event.loc.x), Math.round(event.loc.y), Math.round(event.loc.z), degrees(event.w), formatLoc(event.dest))
	...
	function handleStartSkill(type, event, info, data) {
		...
		if(player.hasNocTan && !nocTanWarning) {
			console.log(`[SkillPrediction] Noctenium Infusion detected in inventory.
		...
		const currentSkillCategory = skillsClient ? get(skillsClient, player.templateId, skill.id, 'category') : null
		...
		function runPassive(passive) {
			...
			switch(passive.type) {
				case 77: if(passive.method === 3) {
					effectScale += Math.max(0, passive.value - 1); 
					break
				}
				case 82:
	...
	mod.hook('S_ACTION_STAGE', 9, {order: -Infinity} ,event => {
		if(isMe(event.gameId)) {
			if(mod.settings.debug.skills) {
				...
				if(mod.settings.debug.loc) strs.push(...[degrees(event.w), formatLoc(event.loc)])

				strs.push(...[event.unk1, event.unk2, event.dest ? event.dest.toString() : 'null', event.target ? 1 : 0])

				if(serverAction)
					strs.push(...[
						dist2DOrUnknown(serverAction.loc, event.loc),
						duration + 'ms',
			...
			if(info) {
				...
				*/
				...
				if(clientAction && clientAction === serverAction && !skillInfo(clientAction.skill)) {
					sendActionEnd(6)
				}
	...
	mod.hook('S_EACH_SKILL_RESULT', 13, event => {
		...
		if(isMe(event.target) && reaction.enable) {
			if(mod.settings.debug.skills) {
				...
				if(mod.settings.debug.loc) strs.push(...[degrees(reaction.w), formatLoc(reaction.loc)])
	...
	if(mod.settings.debug.projectiles) {
	...
	function sendActionStage(opts) {
		...
		if(mod.settings.debug.skills) {
		    console.log('[DEBUG] Action Stage:', {
		        skill: skillId(opts.skill),
		        stage: opts.stage,
		        type: info.type,
		...
		// Only clear action flag for non-movement skills
		myPosition.action = info.type === 'movement' ? true : false
	...
	function sendActionEffects(opts) {
		...
		if(mod.settings.debug.projectiles && info.projectiles)
	...
	function grantCharge(skill, info, stage) {
		...
		// mod.command.message('S_GRANT_SKILL FOR SKILL ' + skill.id)
	...
	function updatePosition(event, action, special) {
	    ...
	    if(mod.settings.debug.skills) {
	        console.log('[DEBUG] Position Update:', {
	            from: myPosition ? {x: Math.round(myPosition.loc.x), y: Math.round(myPosition.loc.y), z: Math.round(myPosition.loc.z)} : null,
	            to: event.loc ? {x: Math.round(event.loc.x), y: Math.round(event.loc.y), z: Math.round(event.loc.z)} : null,
	            action: action,
	            skill: clientAction ? skillId(clientAction.skill) : null
	        })
	    }
	
	    myPosition = {
	        loc: event.loc,
	        w: special ? event.w || myPosition.w : event.w,
	        action
	    }
	}
	...
	function movePlayer(distance, dir) {
	    if(mod.settings.debug.skills) {
	        console.log('[DEBUG] Move Player:', {
	            distance: distance,
	            direction: dir,
	            action: myPosition ? myPosition.action : null,
	            applied: distance && !myPosition.action
	        })
	    }
	...
	// Reload skill configuration from disk
	this.reload = function() {
		if(reloadSkillModules()) {
			skillsCache.clear()
			mod.command.message('<font color="#aaffaa">[Skill Prediction] Skills reloaded successfully.</font>')
			return
		}

		mod.command.message('<font color="#ff7777">[Skill Prediction] Reload failed. Check console for config errors.</font>')
	}
}
...
function dist2DOrUnknown(from, to) {
	if(!from || !to) return '???'
	return decimal(from.dist2D(to), 3) + 'u'
}
