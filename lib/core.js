...
// Module-level variable for skillsClient
...
module.exports = function SPCore(mod) {
	...
	mod.hook('S_LOGIN', 'raw', {order: 100, filter: {fake: null}}, event => {
		skillsCache.clear()
		skillsClient = require(`../config/skills_client/${player.templateId}.json`)
	})

	mod.command.add('pspreloadconfig', () => {
		delete require.cache[require.resolve(`../config/skills_client/${player.templateId}.json`)]
		skillsClient = require(`../config/skills_client/${player.templateId}.json`)
		mod.command.message(`Reloaded config ${player.templateId}.json`)
	})
