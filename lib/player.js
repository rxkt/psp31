...
class SPPlayer {
	...
	constructor(mod) {
		...
		mod.hook('S_LOGIN', 12, HOOK_LAST, event => {
			const skillDataMap = require('../config/skillDataMap');
			this.reset()
			...
			console.log("[Skill-Prediction] Loaded skill data for: " + (skillDataMap[event.templateId] || "Unknown"));
		})
		...
		*/
		...
		{
			let items = null
			mod.hook('S_INVEN', mod.majorPatchVersion >= 74 ? 18 : 17, event => {
